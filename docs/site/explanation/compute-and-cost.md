...
| Databricks App compute | During two bounded attended-installation checks, and after an administrator runs `dbtobsb start` | Installation stops and verifies the App after each check. After a later start, run `dbtobsb stop`; closing the browser is not enough. |
...
Installation starts and stops the App during two bounded deployment checks, then
leaves it stopped with the reconciler schedule paused. The App does not query data
on page load. A viewer must select **Load observability**, which can start the bound
SQL warehouse.
