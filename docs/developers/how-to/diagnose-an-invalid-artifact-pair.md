...
Use this guide when the inspector returns `PAIR_INVALID` or exits `10`.
...
If the error is exit `3`, the tool deliberately emits only `DBTOBSB_INPUT_READ_ERROR`; it never echoes the path. If the error is exit `4`, retain the product version and safe command form, then report `DBTOBSB_INTERNAL_ERROR` without attaching raw artifacts to an ordinary ticket.
