...
`dbtobsb_capture` exposes the stable artifact-pair inspection API plus the v0.5 collector-facing archive APIs. For installation and a guided first result, use [Inspect an artifact pair](../tutorials/inspect-an-artifact-pair.md#4-use-the-python-api).
...
These exports support the customer-local v0.5 collector. They are shipped in the content-addressed `0.5.0` release wheels and remain governed by `dbtobsb.support.v1`.

| Export | Purpose |
| --- | --- |
| `inspect_dbt_output_archive(*, archive: bytes) -> ArchiveCapture` | Inspect one bounded deterministic dbtobsb tar.gz archive without extracting it. |
...
The stable P1.1 report contract below remains `dbtobsb.artifact-pair-report.v1`. Collector projection types are not added to that JSON report.
