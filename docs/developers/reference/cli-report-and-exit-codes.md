...
| `3` | An input could not be read safely. | `DBTOBSB_INPUT_READ_ERROR` on stderr. |
...
| `primary_issue` | First issue under deterministic precedence; otherwise `null`. |
| `issues` | Zero or more static issues, bounded to 20. |

The `summary` contains exact schema URLs, dbt version, adapter type, command, result count, and sorted native-status counts. It contains no resource or invocation identity. Every issue contains only `code`, `component`, `field`, `observed_category`, `impact`, and `next_action`; observed evidence is never interpolated.
...
| Strict JSON | `DBT_MANIFEST_JSON_INVALID`, `DBT_RUN_RESULTS_JSON_INVALID`, `DBT_MANIFEST_JSON_DUPLICATE_KEY`, `DBT_RUN_RESULTS_JSON_DUPLICATE_KEY` |
