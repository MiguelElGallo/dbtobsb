...
| `DBT_COMMAND_UNSUPPORTED` | The result artifact is not evidence from the exact supported named-selector build. | Unsupported compatibility | Run the approved named-selector dbt build and collect that run_results.json. | [Recover pair metadata](../how-to/diagnose-an-invalid-artifact-pair.md#recover-pair-metadata) |
...
| `DBT_TIMING_INVALID` | The artifact contains a timestamp or duration that cannot be stored as execution time. | Recollect | Collect a fresh unmodified run_results.json from the pinned dbt build. | [Recover numeric evidence](../how-to/diagnose-an-invalid-artifact-pair.md#recover-numeric-evidence) |
