...
The v0.3 release does not provide a supported raw-artifact export or automated retention workflow. If policy requires investigation, an authorized custodian should follow the customer's approved Databricks procedure without copying content into an issue, chat, email, or ordinary support ticket.
