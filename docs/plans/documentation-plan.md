...
- how-to: `developers/how-to/diagnose-an-invalid-artifact-pair.md`;
- reference: `developers/reference/python-api.md` and `developers/reference/cli-report-and-exit-codes.md`; and
- explanation: `developers/explanation/pair-validity-vs-dbt-outcome-vs-capture-state.md`.
...
- `how-to/stage-bind-and-accept-the-app-deployment.md` — approve/apply the no-authority stage, capture the full deployment baseline, invoke staging once, automatically reconcile exactly one new `SNAPSHOT`, stop, generate and separately approve a fresh final-binding plan, create a candidate, start the reconciled deployment, reobserve, accept, or stop safely.
...
- `how-to/recover-a-stale-final-binding-plan.md` — return from changed Direct lineage/serial, remote binding drift, interrupted Apply, or intent mismatch to a fresh plan and approval without bypassing validation.
...
| P1.1 developer tutorial, invalid-pair how-to, API/CLI reference, and pair/outcome/capture explanation | Library developer/integrator; P1.1 | D1P; capture-library owner plus dbt documentation owner | `base`; exact runnable fixture commands/output, synthetic-data notice, issue/exit/API contracts, deterministic and installed-wheel tests, and explicit non-claim of dbt success or capture completeness |
...
- Keep Bundle validation, migration envelope, Query History marker, bounded Statement Execution, exact revoke, pending data attestation, composite seal, App stage plan/apply, paginated deployment baseline, single wrapper `bundle run` invocation, stop/reconciliation, fresh final-binding plan/approval/apply, pre-start candidate, direct `apps start`, post-start observation, and final trust event distinct. Explain that the pinned runner may start prior code and internally reissue deployment after an error; never call one invocation one submission. Never describe data operations or trust-event DML as Direct resources. Enumerate runtime DML and universal control-ledger/DDL denials separately.
...
- State that the product schema is customer-owned and typed, Direct has no schema `grants` list, and targeted operations preserve unrelated direct and inherited assignments. Explain the CLI 1.7.0 omitted-principal reconciliation risk in reference/explanation, not as an unexplained warning inside every task.
...
- Combined-mode preflight, all false independence flags, one account-admin actor/profile, Query History cleanup, pending data attestation/composite seal, staged App, native roster task, accepted trust event, ≤20-minute total active work including roster review, and no handoff state—with no credential/path shown.
...
- Approved stage plan with no product authority/user access; every page of the before-set; one wrapper staging invocation; every page of the after-set; exactly one reconciled new terminal matching `SNAPSHOT`; explicit stop; fresh final-binding plan with new lineage/serial and a diff limited to bindings/ACLs; separate approval; pre-start GA observation and native Permissions-roster comparison by the verifier administrator; `TRUST_CANDIDATE`; direct `apps start`; same-deployment post-start observation; final `SNAPSHOT_ACCEPTED`; one-row-per-event ledger rows and sanitized latest-generation summary; complete component array; the pre-start transition-audit, post-start machine-freshness, and original-roster-freshness query-start times; two validity ages, oldest validity-component age, and expiry; and disclosed group/App/Job/SP/admin/committer roots. It visibly says point-in-time, non-independent, and not continuous; no deployment ID is entered or chosen.
...
1. **Subject accuracy** — Databricks and dbt reviewers verify current behavior and exact commands.
...
| D5 | Upgrade, rollback, retention/export/delete, stop/remove/uninstall, warehouse/history cleanup, authority/trusted-root/control-ledger/fence review, trust refresh, mode recovery, and ownership transfer | Six passes plus destructive safety, foreign-grant preservation, exact revoke, pending/composite proof, zero-authority prior-code interval, fully reconciled staged deployment, fresh post-stage final plan, trust/fence-history retain/delete, P6 drain and terminal-run proof before mutation, explicit reopen or irreversible retire, App/workstation restart, three statement times/two validity clocks, original self-anchor source revalidation, unchanged reuse/no expiry extension, required 24-hour roster review, drift handling, mode completion, no-force-open recovery, and deletion verification |
