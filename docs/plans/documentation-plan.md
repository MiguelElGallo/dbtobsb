...
- State that the product schema is customer-owned and typed, Direct has no schema `grants` list, and targeted operations preserve unrelated direct and inherited assignments. Explain the CLI 1.8.0 omitted-principal reconciliation risk in reference/explanation, not as an unexplained warning inside every task.
