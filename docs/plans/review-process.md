...
| P2 | Jobs, archive, exact per-table fixed-DML implementation, disclosed table-`MODIFY` residual schema-alter authority, and honest cross-table snapshot semantics | Missing/partial/malformed evidence plus one immutable AttemptKey-root trust tuple excluded from dbt identity/outcomes | Separate run/capture/current-trust/collector-observed-trust outcomes |
