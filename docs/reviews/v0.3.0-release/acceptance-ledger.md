...
- Status: Contract freeze passed; implementation gate in progress
- Review model: [Continuous domain ownership](../../plans/review-process.md)
- Candidate contract: [v0.3.0 supported-release contract](../../releases/v0.3.0-support-contract.md)
...
| `CF-004` | First useful evidence and qualification matrix | dbt Core | `DBT-05`, `DBT-06`, `DBT-08`; `UX-03`, `UX-05`, `UX-06` | Freeze separate dbt, pair/capture, node, collection, trust, and next-action results. Freeze cost-before-query and explicit Load behavior. Separate local golden cases from the live release proof. | Fixtures cover success, dbt failure, early/partial evidence, retry/repair, missed collection, replay, reconciliation, trust hold, and empty state; no evidence query occurs before Load. | One immutable RC proves two changed-input complete runs, one selected failure or partial case, missed collection plus reconciliation, distinct histories, queryable rows, and matching App results. |

## Domain decisions

| Domain | Contract-freeze state | Current decision scope |
|---|---|---|
| Databricks platform | `PASS` | Platform support, authority, lifecycle, and the literal custom-profile host binding are frozen for the implementation gate |
| dbt Core | `PASS` | Runtime, artifact, profile/target, and real-project onboarding promises are frozen for the implementation gate |
| Usability | `PASS` | Installation, lifecycle, first-value, and zero-guess generated-profile journeys are frozen for the implementation gate |
...
## Deferred without duplicating findings

- Landing-page links, historical-command quarantine, rendered documentation, accessibility, and prose checks belong to documentation freeze and local RC.
- Reproducible signed artifacts, live installation, runtime evidence, lifecycle rehearsals, paginated inventories, and zero product-started compute belong to the local RC or live Azure gate shown above.
- A later-gate failure reopens the affected outcome and rubric check. It does not create a second finding for the same root cause.

## Closure rule

Each outcome closes when its current-gate evidence is frozen and the named domain owners sign only their columns. A later implementation or live-proof column remains visibly pending until its own milestone; it does not block an earlier gate unless it contradicts the frozen contract.
